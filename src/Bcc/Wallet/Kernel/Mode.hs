...
import           Bcc.Wallet.WalletLayer (PassiveWalletLayer, applyBlocks,
                     rollbackBlocks)
...
instance MonadBListener WalletMode where
  ...
  onRollbackBlocks _nm _pc bs = getWallet >>= (`walletRollbackBlocks` bs)
