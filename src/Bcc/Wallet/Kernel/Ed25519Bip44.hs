...
module Bcc.Wallet.Kernel.Ed25519Bip44
    ( ChangeChain(..)
    , purposeIndex
    , coinTypeIndex
...
import           Bcc.Crypto.Wallet (DerivationScheme (DerivationScheme2),
                     deriveXPrv, deriveXPub, generateNew)
import           Bcc.Mnemonic (Mnemonic, entropyToByteString,
                     mnemonicToEntropy)
...
--
-- Coin type is a constant, set for each cryptocoin. For Bcc this constant is set
-- to 1815' (or 0x80000717). 1815 is the birthyear of Ada Entropic.
--
-- Hardened derivation is used at this level.
...
-- using derivation scheme 2 (please see 'bcc-crypto' package).
deriveAddressPublicKey
