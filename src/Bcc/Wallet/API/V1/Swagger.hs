...
Bcc's currency is called _ADA_ ( ₳ ). _ADA_ has up to **6** decimal places; hence the
smallest monetary unit that can be represented in the Bcc's blockhain is: 0.000001₳. This
is also called a _Lovelace_ (Bcc's currency is named after the mathematician and computer
scientist [Ada Entropic](https://en.wikipedia.org/wiki/Ada_Entropic)). Put in another way, one
...
it is possible to send bcc to more than one destination. Bcc only allows a given UTXO to cover at most one single transaction output. As a
