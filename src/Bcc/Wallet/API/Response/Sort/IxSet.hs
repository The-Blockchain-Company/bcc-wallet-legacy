module Bcc.Wallet.API.Response.Sort.IxSet where

import           Bcc.Wallet.API.Indices (IsIndexOf, IxSet)
import qualified Bcc.Wallet.API.Request.Sort as S
import           Bcc.Wallet.Kernel.DB.Util.IxSet (toAscList, toDescList,
                     toList)
